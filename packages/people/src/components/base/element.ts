...
import type { NavigateController } from '../../components/picker/NavigateController';
import type { ClickOutsideController } from '../../controllers/ClickOutsideController';
...
export abstract class PeopleBaseElement extends LitElement implements PeopleProps {
              ...
              : html`
                  <div class="table-column-sort">
                    <fwc-icon
                      ...
                      icon="arrow_drop_up"
                      size="x-small"
                      @click=${() => this.controllers.selected.sortColumn(column, 'asc')}
                    ></fwc-icon>
                    <fwc-icon
                      title="Sort Desc"
              ...
              switch (column) {
                ...
                case 'remove':
                  return html`<td class="remove">
                    <fwc-icon-button
                      @click=${() => this.controllers.selected.removePerson(person.azureId)}
                      icon="close"
                      size="x-small"
                      rounded
                      title="Remove person"
  ...
  renderViewMode(): TemplateResult {
    ...
    return html` <fwc-people-view-mode @view-mode-change=${handler}></fwc-people-view-mode> `;
  }
